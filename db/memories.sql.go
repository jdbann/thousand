// Code generated by sqlc. DO NOT EDIT.
// source: memories.sql

package db

import (
	"context"

	"github.com/google/uuid"
	"github.com/lib/pq"
)

const createMemories = `-- name: CreateMemories :many
INSERT INTO memories (vampire_id)
SELECT
    unnest($1::uuid[]) AS vampire_id
RETURNING
    id, vampire_id, created_at, updated_at
`

func (q *Queries) CreateMemories(ctx context.Context, vampireID []uuid.UUID) ([]Memory, error) {
	rows, err := q.db.QueryContext(ctx, createMemories, pq.Array(vampireID))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Memory
	for rows.Next() {
		var i Memory
		if err := rows.Scan(
			&i.ID,
			&i.VampireID,
			&i.CreatedAt,
			&i.UpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const createMemory = `-- name: CreateMemory :one
INSERT INTO memories (vampire_id)
    VALUES ($1)
RETURNING
    id, vampire_id, created_at, updated_at
`

func (q *Queries) CreateMemory(ctx context.Context, vampireID uuid.UUID) (Memory, error) {
	row := q.db.QueryRowContext(ctx, createMemory, vampireID)
	var i Memory
	err := row.Scan(
		&i.ID,
		&i.VampireID,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}

const getMemoriesForVampire = `-- name: GetMemoriesForVampire :many
SELECT
    id, vampire_id, created_at, updated_at
FROM
    memories
WHERE
    vampire_id = $1
`

func (q *Queries) GetMemoriesForVampire(ctx context.Context, vampireID uuid.UUID) ([]Memory, error) {
	rows, err := q.db.QueryContext(ctx, getMemoriesForVampire, vampireID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Memory
	for rows.Next() {
		var i Memory
		if err := rows.Scan(
			&i.ID,
			&i.VampireID,
			&i.CreatedAt,
			&i.UpdatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const getMemory = `-- name: GetMemory :one
SELECT
    id, vampire_id, created_at, updated_at
FROM
    memories
WHERE
    id = $1
LIMIT 1
`

func (q *Queries) GetMemory(ctx context.Context, id uuid.UUID) (Memory, error) {
	row := q.db.QueryRowContext(ctx, getMemory, id)
	var i Memory
	err := row.Scan(
		&i.ID,
		&i.VampireID,
		&i.CreatedAt,
		&i.UpdatedAt,
	)
	return i, err
}
